$(function() {
  ...
  $(document).ready(function () {
    ...
    function fetchAndRefreshChart() {
        $.ajax({
            url: '/', // Replace with your actual route URL
            type: 'GET',
            dataType: 'json',
            success: function (data) {
              var labels = data.map(row => {
                var monthNumber = row.month;
                var monthName = new Date(0, monthNumber - 1).toLocaleDateString('en-US', { month: 'long' });
                ...
                if (apexBarChart) {
                    ...
                    var apexBarChart = new ApexCharts(document.querySelector("#"), options);
                    apexBarChart.render();
                }
            },
    ...
    setInterval(fetchAndRefreshChart, 30000); // Adjust the interval as needed
});

// Monthly Sinistres Dim Chart
$(document).ready(function () {
  // Function to fetch data and update the chart
  function fetchAndRefreshChart() {
      $.ajax({
...
// Monthly Sinistres AT and RD Chart
$(document).ready(function () {
  // Function to fetch data and update the chart
  function fetchAndRefreshChart() {
      $.ajax({
  ...
  //     series: [{
  //       name: 'Production',
  //       data: [15,19,93,13,12,1,18,43,2,11,11,12]
  //     }],
  //     xaxis: {
  //       type: 'datetime',
